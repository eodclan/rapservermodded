...
class Prison {
	constructor() {
		mp.events.add({
			"playerDeath" : (player, reason, killer) => {
				...
				this.addViolation(killer, 5, "Du hast einen Zivilisten getötet");
			},
	...
	async loadUser(player) {
		...
		player.startJail = function() {
			if (this.jail.violations.length === 0) return this.notify("Du hast keine Verstöße!");
			player.tp(building.secondEntranceData.inPos);
			let jailTime = 0;
			for (const violation of this.jail.violations) {
				this.notify(`${violation.text} | ${violation.time} Minuten`);
				jailTime += violation.time;
			}
			this.notify(`Gesamtzeit: ${jailTime} Minuten`);
			this.jail.inside = 1;
		...
		player.jailEvent = function() {
			...
			this.notify(`Verbleibende Zeit: ~r~${this.jail.time} Minuten`);
			misc.log.debug(`${this.name} Verbleibende Gefängnis Zeit: ${this.jail.time} m`);
		}
		...
		player.endJail = function() {
			...
			this.heading = 263.45;
			this.jail.inside = 0;
			this.notify(`Jetzt bist du frei!`);
			this.updateWantedLevel();
