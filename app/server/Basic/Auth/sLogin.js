...
class LoginSingleton extends AbstractAuth {
    async tryLoginWithoutCode(player, obj) {
        ...
        if (d[0].password !== pass) {
            ...
            this.showError(player, `Sie können sich nicht von zwei Geräten aus einloggen!`);
    ...
    async loadAccount(player, id) {
        ...
        player.notify(`${i18n.get('sLogin', 'annouceSpawnVehicle', player.lang)}`);
        player.notify(`${i18n.get('sLogin', 'annouceGlobalChat', player.lang)}`);
        player.notify(`${i18n.get('sLogin', 'annouceOldUser', player.lang)}`);
        player.notify(`${i18n.get('sLogin', 'annoucePlayerMenu', player.lang)}`);
	mp.nametags.enabled = false;
        const onlinePlayers = mp.players.toArray();
        if (onlinePlayers.length < 30) {
            for (const p of onlinePlayers) {
                p.notify(`[${misc.getTime()}] ${player.name} ${i18n.get('sLogin', 'connected', p.lang)}`);
            }
        }
    }
...
mp.events.add({
    ...
    "playerQuit" : (player) => {
        ...
        if (onlinePlayers.length < 30) {
            for (const p of onlinePlayers) {
                p.notify(`[${misc.getTime()}] ${player.name} ${i18n.get('sLogin', 'disconnected', p.lang)}`);
            }
        } 
    },
});