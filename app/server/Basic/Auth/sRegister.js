...
class RegiserSingleton extends AbstractAuth {
    ...
    async tryGetCodeToRegister(player, email) {
        ...
        if (d[0]) {
            return this.showError(player, "Diese E-Mail existiert bereits");
        }
    ...
    async checkUsername(player, obj) {
        ...
        if (d[0]) {
            return this.showError(player, "Dieser Nickname existiert bereits");
        }
    ...
    async tryCreateAccount(player, obj) {
        ...
        if (!mailer.isEmailValid(data.email)) {
            return this.showError(player, "E-Mail ungültig");
        }
        const d = await misc.query(`SELECT email FROM users WHERE email = '${data.email}' LIMIT 1`);
        if (d[0]) {
            return this.showError(player, "Etwas stimmt nicht. Versuchen Sie es nochmal");
        }
    ...
    async createAccount(player, d) {
        ...
        player.call("cInjectCef", [`app.showInfo('Erfolg! Nun bitte Einloggen!');`]);
    }
